...
#   thesslink_e3_w6_nav_v1_g64  (navigation-only,  v6_nav,  t_limit=640)
#
# (No w7 full g64 is registered; use train_g32.sh for full-episode g32.)
#
# Usage:
#   ./train_g64.sh            # run in foreground
#   ./train_g64.sh --detach   # re-exec under nohup and exit
#
...
if [[ "${1:-}" == "--detach" ]]; then
    ...
    echo "[train_g64] Detaching → $launcher"
    nohup bash "$SCRIPT_DIR/train_g64.sh" "$@" >>"$launcher" 2>&1 &
    echo "[train_g64] Started pid=$! — safe to close this terminal. tail -f $launcher"
    exit 0
fi

if [[ $# -gt 0 ]]; then
    echo "[train_g64] unknown argument: $1 (only --detach is accepted)" >&2
    exit 1
fi
...
log()  { echo -e "\033[1;32m[train_g64]\033[0m $*"; }
err()  { echo -e "\033[1;31m[train_g64]\033[0m $*" >&2; }
