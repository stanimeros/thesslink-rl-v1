...
3. Per-agent shaping normalisation.  v5 normalised the potential by the global
   ...
   which pairs well with the 25.0 arrival reward.

Observation layout (size 12):
...
# Minimal nav obs: self_pos(2) + relative_pos(2) + lidar_card(4) + lidar_diag(4)
OBS_FLAT_SIZE_V6 = 12
...
class GridNegotiationGymEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.observation_space = spaces.Tuple(
            tuple(
                spaces.Box(low=-1.0, high=1.0, shape=(OBS_FLAT_SIZE_V6,), dtype=np.float32)
    ...
    def _get_nav_obs(self, agents: list[str]) -> tuple[np.ndarray, ...]:
        """Build minimal nav obs: self_pos + relative_pos + 8-dir lidar (size 12)."""
        obs_list = []
        ...
        for a in agents:
            ...
            obs_list.append(np.concatenate([self_pos, relative_pos, card, diag]))
